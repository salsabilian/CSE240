...
class SnakeGame:
    ...
    def do_training(self):
        ...
        for game in range(1, self.args.NUM_TRAIN_ITER + 1):
            ...
            state = self.env.get_state()  # starting state for agent_action
            dead = False
            points = 0
            action = self.agent.agent_action(state, points, dead)
            while not dead: # keep running while were not dead
                state, points, dead = self.env.step(action)  # do the action specified by agent action and return the next state
                if points >= 1:  # if we score any points save it as a viable path in the model
                    self.agent.save_model()
                action = self.agent.agent_action(state, points, dead) # then figure out the next best action
            points = self.env.get_points()  # get the points at the end for this run through
            self.points_results.append(points)  # append it to the points results
    ...
    def do_testing(self):
        ...
        for game in range(1, self.args.NUM_TEST_ITER + 1):
            ...
            state = self.env.get_state()  # starting state for agent action
            dead = False
            points = 0
            action = self.agent.agent_action(state, points, dead)  # get the best action at start
            while not dead: # keep going till we are dead
                state, points, dead = self.env.step(action)  # do the move decided by agent_action
                action = self.agent.agent_action(state, points, dead)  # get the best action again
            points = self.env.get_points()  # get our final number of points
            points_results.append(points)  # append the points
            self.env.reset()  # reset the game
            # YOUR CODE HERE
            # YOUR CODE HERE
            # YOUR CODE HERE
